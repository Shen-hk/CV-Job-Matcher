...
| 长期目标运行时 | 把求职目标拆成计划和任务，记录工具结果，并持续推荐下一步行动 |
| Agent Supervisor | 检查计划健康度、识别阻塞、切换可并行任务，并判断继续执行或重新规划 |
...
长期任务由执行器和 Supervisor 共同形成闭环：执行器调用求职工具并沉淀证据，Supervisor 根据真实状态输出继续、阻塞、复盘或停止决策。完成一轮计划不会被误判为拿到 Offer，目标达成必须由用户明确确认。
...
TieLink 还维护一条可持久化的长期执行链：

```mermaid
flowchart LR
    G["Career Goal"] --> P["Plan"]
    P --> T["Next Task"]
    T --> X["Tool Execution"]
    X --> O["Observation"]
    O --> D{"继续或调整"}
...
目标、计划版本、任务状态和观察记录保存在本地。JD、匹配、简历、面试和投递工具完成后，会自动把结果回写到当前计划；涉及完成确认或重新规划时仍由用户明确触发。
