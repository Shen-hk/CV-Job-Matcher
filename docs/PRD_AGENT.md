...
- 在音轨转录之外，增加“表达分析”层：语速、停顿、口头禅、回答时长、打断频率
- 在文本分析之外，增加“回答合规”层：是否跑题、是否夸大、是否缺少事实支撑、是否触发敏感表达风险
- 在视频分析层，谨慎输出“提醒级”非语言结论，如长时间低头、明显紧张、视线飘忽、表情僵硬，不做过度确定的人格判断
- 复盘结果直接联动后续动作：生成简历改写建议、打招呼话术优化建议、下一轮模拟面试训练重点
